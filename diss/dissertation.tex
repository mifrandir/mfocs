...
\begin{lemma}\label{lem:affine_space_of_dolbeault_operators}
  ...
  \end{align*}
  Then $\dol_E\in A^{\dol}$ if, and only if, $\dol_E+\alpha\in A^{\dol}$.
  \begin{proof}
...
Thus we may write $A^{\dol} = \dol_E + \Omega^{0,1}(C,\End E)$
for some fixed $\dol_E$. This induces a topology on $A^{\dol}$.
In particular, $A^{\dol}$ is an affine space modelled on
$\Omega^{0,1}(C,\End E)$.

This allows us to define the space of all Higgs bundles
\begin{align*}
  A^\phi := \bigsqcup_{\dol_E \in A^{\dol}} \Gamma(C,\End(E,\dol_E)\otimes K_C)
\end{align*}
whose topology is induced by the topologies on
$\Gamma(C,\End(E,\dol_E)\otimes K_C)$ and $A^{\dol}$.
...
\end{equation}
\begin{lemma}
  The topological space $A^{\dol}/\Aut(E)$ of isomorphism classes of
...
Note that we have a similar action of $\Aut(E)$ on $A^\phi$ and
...
over $A^{\dol}$ must also be non-Hausdorff. Therefore we need
...
Connections are needed for two reasons: Firstly, they are going to
be equivalent to Dolbeault operators and hence lead to an
alternative way of looking at spaces of holomorphic bundles.
...
While connections are very closely related to Dolbeault operators,
they are not quite the same. To map one to the other, we are going
...
Fix such a metric $h$ on $E$. The main thing we gain by having a
metric is the ability to refer to adjoints and hence unitary
endomorphisms of $E$. In particular, there is a principal $U(r)$-bundle
$U(E)$ whose fibre over $x\in C$ is the group of $h$-unitary
endomorphisms of $E_x$. Noting that $U(n)$ is a real Lie group,
we obtain a real smooth vector bundle $\mathfrak u(E)$ on $C$.
In particular, we have a sequence of inclusions
\begin{align*}
  \Omega^1_{\mathbb{R}}(C,\mathfrak u(E))
  \subseteq \Omega^1_{\mathbb{R}}(C,\mathfrak{gl}(E))
  \subseteq \Omega^1(C,\mathfrak{gl}(E))
\end{align*}
where $\mathfrak{gl}(E)$ is the smooth complex vector bundle obtained
by taking Lie algebras of the principal $GL_r(\mathbb{C})$-bundle $GL(E)$.
Now note that a connection is equivalently a section in
$\Omega^1(C,\End E)$. We may regard $\End E$ as $\mathfrak gl(E)$,
and hence define what it means for a connection to be unitary:
\todo{needs rewriting}
...
\end{align*}
Analogous to $A^{\dol}$, we have the following property:
...
similarities between $A^{\dol}$ and $A^\nabla(E,h)$ grow even more
striking:
...
It is not a coincidence that $A^{\dol}$ and $A^\nabla(E,h)$ both are
...
$\nabla^h : A^{\dol} \to A^\nabla$ whose inverse is the restriction
...
Now we have an action of $\Gamma^\infty(C,U(E))$ on
$A^\nabla(E,h)$ via conjugation. That is,
for $g\in\Gamma^\infty(C,U(E))$ and
$\nabla\in A^\nabla(E,h)$, $g\cdot\nabla$ is the map
\begin{align*}
...
However, note that there is some redundancy because scalar multiplication
commutes with the connection. That is, if $g:E\to E$ is multiplication
by a scalar then $g\cdot\nabla = \nabla$. Hence we also have an action of
\begin{align*}
  G(E,h) := \Gamma^\infty(C,U(E)) / U(1)
\end{align*}
on $A^\nabla(E,h)$.
...
We have successfully viewed the space of holomorphic bundles $A^{\dol}$
as an affine space modelled on
$\Omega^1_{\mathbb{R}}(C,\mathfrak{u}(E))$. This is eventually going
to help us introduce a symplectic structure on $A^{\dol}$ and hence
take a symplectic quotient. Let us do the same for the space of Higgs
bundles $A^\phi$. We are going to identify $A^{\phi}$ with an affine
...
\begin{align*}
  A^\phi \subseteq A^{\dol} \times \Omega^{1,0}(\End E) \cong A^\Phi.
\end{align*}
Note that in general $A^\phi$ and $A^\Phi$ are not isomorphic. In
...
Before we move into the realm of infinite dimensional Banach spaces,
let us study the group actions a bit more closely. Later we are going to
require our actions to be free and proper. Let us recall what that
means.
...
\begin{example}
  ...
  In particular, the action of $U(1)$ on $\Gamma^\infty(C,U(E))$ is
  proper.
\end{example}

\begin{example}\label{ex:h_proper_action}
  The action of $U(E)$ on $A^\nabla$ is proper.
  \begin{proof}
    ...
    group. The next step is to fix a point $x_0\in X$ and
    $p_0\in P_{x_0}$ and consider curves $c$ starting at $x_0$ in $X$.
    Considering $\nabla$ and $\nabla'$ yields lifts $\tilde c$ and
...
\begin{example}\label{ex:higgs_proper_action}
  the action of $G_{\ell+1}(E,h)$ on $A_\ell^\Phi$ is proper.
  \missingexample
\end{example}
...
and $A^\Phi$ with manifold structures. Of course, both are affine
...
\begin{example}
  ...
  us to define
  \begin{align*}
    \Omega_{\mathbb{R},\ell}^k(X,E) := W_{2,\ell}(X,E\otimes(T^*X)^{\wedge k}).
  \end{align*}
\end{example}
...
\begin{example}
  ...
  $A^\nabla$ modelled on $\Omega^1_{\mathbb{R}}(C,\mathfrak u(E))$.
  For each $\ell \geq 0$, we have the corresponding Sobolev completion
  \begin{align*}
    A^\nabla_\ell(E,h) = A^\nabla(E,h) + \Omega^1_{\mathbb{R},\ell}(C,\mathfrak u(E)).
  \end{align*}
  In particular, for each $\nabla\in A^\nabla_\ell$,
  \begin{align*}
    T^*_\nabla A^\nabla_\ell
    = \Omega^1_{\mathbb{R},\ell}(C,\mathfrak u(E)).
  \end{align*}
\end{example}

\begin{example}\todo{be a bit more careful with base space and metrics}
  Consider the group $\Gamma^\infty(C,U(E))$ of unitary
  sections of $\End E$. The Sobolev norms on $\Gamma^\infty(C,\End E)$
  restrict to norms on the subset $\Gamma^\infty(C,U(E))$ and hence we
  may consider the completions $W_{2,\ell}(C,U(E))$.
  One may show that this is a Banach manifold by applying the
  analogue of the regular value theorem. In particular, for all
  $A\in W_{2,\ell}(C,U(E))$, we have
  \todo{neitzke says it's $\mathfrak{gl}(E)$ but i'm not a fan}
  \begin{align*}
    T_A W_{2,\ell}(C,U(E)) = W_{2,\ell}(C,\mathfrak u(E)).
  \end{align*}
\end{example}
...
\begin{example}
  The Banach manifold $W_{2,\ell}(C,U(E))$ is a Banach Lie group.
...
\begin{example}
  The Lie algebra of $W_{2,{\ell}}(C,U(E))$ is
  $W_{2,\ell}(C,\mathfrak u(E))$.
\end{example}
...
\begin{example}
  As a primitive example, the finite-dimensional Banach Lie group
  $U(1)$ acts on the Banach Lie group $W_{2,\ell}(C,U(E))$ by
  multiplication: $\lambda \cdot A = \lambda A$.
\end{example}

Perhaps more interestingly, Sobolev completions allow us to lift group
actions:

\begin{example}
  Consider the action of $\Gamma^\infty(C,U(E))$ on $A^\nabla$
  by conjugation. This extends to an action of $W_{2,\ell+1}(C,U(E))$
  on $A^\nabla_\ell$.
  \missingproof
  Analogously we have a Banach Lie group action of $W_{2,\ell+1}(C,U(E))$
  on $A^\Phi_\ell$.
\end{example}
...
\begin{example}
  the action of $G_{\ell+1}(E,h)$ on $A_\ell^\nabla$ is not free.
  \missingexample
\end{example}

\begin{example}
  the action of $G_{\ell+1}(E,h)$ on $A_\ell^\Phi$ is not free.
  \missingexample
\end{example}
...
\begin{definition}
  A \emph{symplectic form} on a Banach manifold $X$ is a 2-form
  $\omega\in\Omega_{\mathbb{R}}^2(X)$ such that $d\omega = 0$
  and, for every $x\in X$, there is an injection $T_x X\inc T_x^* X$
  given by $v \mapsto \omega(v,-)$.
\end{definition}

Note if $X$ is finite dimensional then $T_x X \cong T_x^*X$ so the
map $v \mapsto \omega(v,-)$ must be an isomorphism.
...
\begin{definition}
  Consider a symplectic form $\omega$ on a Banach manifold $X$
  and an action $\rho : G\times X\to X$ of a Banach Lie group.
  A \emph{moment map} for $\rho$ is a map
  \begin{align*}
    \mu : X \to \mathfrak{g}^*
  \end{align*}
  such that, for all $Z\in\mathfrak g$, the function
  $\mu_Z$ given by $x \mapsto\langle\mu(x),Z\rangle$ satisfies
  \begin{align*}
    d\mu_Z = \iota_{\rho(Z)}\omega.
  \end{align*}
\end{definition}

\begin{example}\todo{define curvature}
  A moment map for the action of $G_{\ell+1}(E,h)$ on $A^\nabla_\ell$
  is the map
  \begin{align*}
    \mu : A^\nabla_\ell \to \Omega^0_{\mathbb{R},\ell+1}(\End E)^*
  \end{align*}
  given by
  \begin{align*}
    \mu_Z(D) = - \int_C \Tr(ZF_D) - 2\pi i\frac{d}{K}\int_C \omega_C\Tr(Z).
  \end{align*}
  Here the map
  \begin{align*}
    \Tr : \Omega^k_{\mathbb{R}}(C,\End E) \to \Omega^k_{\mathbb{R}}(C)
  \end{align*}
  is induced by the map $E^*_x \otimes_{\mathbb{R}} E_x \to \mathbb{R}$
  given by $\xi\otimes v \mapsto \xi(v)$. Moreover,
  $\omega_C\in\Omega^2_{\mathbb{R}}(C)$ is the symplectic form on $C$
  and $F_D = D\circ D\in\Omega^2_{\mathbb{R}}(\End E)$ is the curvature of
  $D$.
\end{example}

\begin{example}
  moment map for $A^\Phi$
  \missingexample
\end{example}

\subsection{Sympletic Quotients}

Consider a moment map $\mu$ for a proper action of a Banach Lie group
$G$ on a symplectic Banach manifold $(X,\omega)$. We are finally ready
to define what it means to be a quotient in this setting:

\begin{definition}
  The \emph{symplectic quotient} of $X$ by $G$ is
  \begin{align*}
    X\sslash G :=  {\mu}^{-1}(0)/G.
  \end{align*}
\end{definition}

Of course, a the moment we have gained little to nothing from this
definition. The quotient makes sense as a topological space, but nothing
else. Firstly, we want this quotient to be a Banach manifold. The obvious
result to use is \ref{thm:banach_quotient}. Let us make sure that
the conditions are satisfied.

\begin{example}
  Consider the action of $G_{\ell+1}(E,h)$ on $A^\nabla_\ell$.
  We already know from \ref{ex:h_proper_action} that this action
  is proper.
\end{example}

\begin{theorem}\label{thm:symplectic_quotient}
  If
  \begin{enumerate}
    \item $Y:={\mu}^{-1}(0)$ is a Banach submanifold of $X$,
    \item $G$ acts freely on $Y$,
    \item each tangent space $T_x(Gx)$ to $G$-orbits is closed,
      complemented, and obeys
      \begin{align*}
        T_x(Gx) = (T_x(Gx)^{\bot})^\bot
      \end{align*}
  \end{enumerate}
  then $Y/G$ is a Banach manifold with a symplectic form
  $\omega_{Y/G}$ satisfying
  \begin{align*}
    q^*\omega_{Y/G} = \iota^*\omega
  \end{align*}
  where $q : Y\surj Y/G$ is the quotient map
  and $\iota : Y\inc X$ is the inclusion.
  \begin{proof}
    \missingproof
  \end{proof}
\end{theorem}
...
\begin{example}
  We are now ready
  \begin{align*}
    N^\omega(E) := A^\nabla_{\ell} \sslash G_{\ell + 1}(E,h).
  \end{align*}
\end{example}
\begin{example}
  On the other hand, the analytic moduli space of Higgs bundles on
  $C$ with underlying smooth bundle $E$ is
  \begin{align*}
    M^\omega(E) := A^\Phi_\ell \sslash G_{\ell + 1}(E,h).
  \end{align*}
\end{example}
