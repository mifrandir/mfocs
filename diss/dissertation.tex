...
\begin{example}\label{ex:quot_scheme_of_higgs}
  ...
  Note that $Q^\circ(r,d)$ is a reduced open subscheme
  ...
  $C\times Q^\circ(r,d)$ we find that the functor
  \begin{align*}
    (f:T\to Q(r,d))
    \mapsto
    \Gamma(C\times T,(\identity\times f)^*\Hom(\hat{\mathscr E},\hat{\mathscr E}\otimes\pi^*\mathscr K))
  \end{align*}
  where $\pi : C\times Q^\circ(r,d)\to C$ is represented by a
  $Q^\circ(r,d)$-scheme $\mathcal F(r,d)$.
  In particular, a closed point of $\mathcal F(r,d)$ corresponds to a
  closed point $q\in Q^\circ(r,d)(\mathbb C)$ and a $q$-point of $\mathcal F(r,d)$.
  We may now choose $q$ such that $\mathscr E = \hat{\mathscr E}_q$ and note
  that
  \begin{align*}
    \mathcal F(r,d)(q) = \Gamma(C,\Hom(\mathscr E,\mathscr E\otimes\mathscr K)).
  \end{align*}
  ...
  \end{align*}
  Thus $\varphi$ corresponds to an $q$-point of $\mathcal F(r,d)$
...
\begin{theorem}
  ...
  \begin{proof}[Proof idea]
    ...
    Conditions 3 and 4 are are addressed by considering
...
\begin{example}
  The $GL_n$-linearised ample sheaf $\mathscr L^\ell_h$ on $Q(r,d)$
  pulls back along $u:F(r,d)\to Q^\circ(r,d)$ so we have a line bundle
  $\mathscr L^\ell_H$ on $F(r,d)$. The linearisation of
  $\mathscr L^\ell_h$ extends to $\mathscr L^\ell_H$ by
  $GL_n$-equivariance of $u$. \todo{ampleness?}
\end{example}
...
\begin{example}
  ...
  \begin{align*}
    M^{ss}_h(r,d) := Q(r,d)\sslash_{{\mathscr L}^m_h} GL_n, \hspace{1cm}
    M^{ss}_H(r,d) := F(r,d)\sslash_{{\mathscr L}^m_H} GL_n.
  \end{align*}
  \missingexample
\end{example}
...
\begin{example}
  ...
  \begin{align*}
    M^{s}_h(r,d) := Q(r,d)^s\sslash_{L^m_h} GL_n, \hspace{1cm}
    M^{s}_H(r,d) := F(r,d)^s\sslash_{L^m_H} GL_n.
  \end{align*}
...
By using the power of geometric invariant theory, we have been able
...
find schemes $Q^\circ(r,d)$ and $F(r,d)$ parametrising the sheaves
...
but cannot yet be sure which orbits in $Q^\circ(r,d)$ and $F(r,d)$
are represented.
