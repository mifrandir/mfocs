...
\begin{theorem}
  ...
  \begin{proof}[Proof idea]
    ...
    \begin{align}\label{eq:quot_embedding}
      ...
      \longinc
      Gr(H^0(\mathscr O^{\oplus r}_{\projective{n}{}}),p(m))
      \longinc
      \projective{}{}(\bigwedge^{p(m)}H^0(\mathscr O^{\oplus r}_{\projective{n}{}}))
    \end{align}
...
\begin{example}\label{ex:quot_scheme_of_higgs}
  ...
  Consider the universal sheaf $\hat{\mathscr E}$ on $Q(r,d)$
  arising from the universal quotient
  \begin{align*}
    \hat q : \mathscr O^{\oplus n}_{C\times Q(r,d)} \to \hat{\mathscr E}.
  \end{align*}
  Applying the lemma \ref{lem:linear_scheme} to the sheaf
  $\Hom(\hat{\mathscr E},\hat{\mathscr E}\otimes K)$ on $C\otimes Q(r,d)$
...
\begin{example}
  ...
  $Q(r,d)$ from \ref{ex:lf_action}. We follow
  ...
  \begin{align*}
    (\sigma\times\identity)^*\hat{\mathscr E} \cong \pi^*\hat{\mathscr E}.
  \end{align*}
  The line bundle on $Q(r,d)$ to consider then arises
  by pulling back $\mathscr O(1)$ along the embedding (\ref{eq:quot_embedding}).
  In particular, for $m$ sufficiently large, we have an
  embedding
  \begin{align*}
    Q(r,d) \longinc
    \projective{}{}(\wedge^{mr+d+r(1-g)} H^0(\mathscr O^{\oplus n}_C(m))^\vee).
  \end{align*}
  The pullback of $\mathscr O(1)$ along this embedding is \todo{why?}
  \begin{align*}
    \mathscr L^m_h := \det(\pi_*(\hat{\mathscr E} \otimes \pi^*\mathscr O_C(m)))
  \end{align*}
  which, for $m$ large, is an ample sheaf on on $Q(r,d)$.
  That is, for $m$ sufficiently large, 
  on $Q(r,d)$. That is, there is an isormorphism
  \begin{align*}
    \Phi:\sigma^*\mathscr L^m_h \cong \pi^*\mathscr L^m_h
  \end{align*}
  of sheaves on $GL_n\times Q(r,d)$ such that \todo{check what sides the identities are on}
  \begin{align*}
...
\begin{example}
  ...
  action on $Q(r,d)$, this is unsurprising.
...
\begin{theorem}[{\cite[Theorem C]{fan2020}}]
  \begin{align*}
