...
\section{Constructing Geometric Quotients}
...
\subsection{Affine Good Quotients}
...
\subsection{Affine Geometric Quotients}

Consider an affine GIT quotient $\phi : X \surj X\sslash G$.
...
\begin{lemma}
  $X^s\sslash G := \phi(X^s)$ is open in $X\sslash G$ and
  $\phi^{-1}(X^s\sslash G) = X^s$.
  \begin{proof}
    \missingproof
  \end{proof}
\end{lemma}
...
\begin{theorem}
  The restriction $\phi : X^s \surj X^s\sslash G$ is a geometric quotient.
  \begin{proof}
    \missingproof
  \end{proof}
\end{theorem}
...
\subsection{Quotients by Linearised Actions}
