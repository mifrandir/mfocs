...
\begin{definition}\missingcitation
  ...
  \begin{itemize}
    ...
    \item every natural transformation $\nu : M \to \Hom(-,M')$ factors through
...
for all $t$ in the same connected component.
...
$f : T' \to T$ is a base change then we have an equality
...
Now note that this functor splits into
\begin{align*}
  \mathcal Quot(\mathscr E)
  = \bigsqcup_{P\in\mathbb{Q}[t]} \mathcal Quot(\mathscr E)^{P,\mathscr L}
\end{align*}
where each $\mathcal Quot(\mathscr E)^{P,\mathscr L}$ contains families
...
\begin{example}
  Recall that the Hilbert polynomial of a locally free sheaf on $C$
  with respect to a fixed invertible sheaf depends only on its rank and degree.
  Thus, for any locally free sheaf $\mathscr E$ on $C$, we have a splitting
  \begin{align}\label{eq:quot_functor_split_on_c}
    \mathcal Quot(\mathscr E) = \bigsqcup_{r,d} \mathcal Quot(\mathscr E)^{r,d}.
  \end{align}
  \missingexample
\end{example}
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    into projective space is an immersion \missingproof
...
\begin{example}
  ...
  i.e. a closed point of the scheme
  \begin{align*}
...
are pairs $(\mathscr E,\phi)$ where $\mathscr E\in Q(r,d)$
and $\phi : \mathscr E \to \mathscr E \otimes \mathscr K$.
Now a Higgs field $\phi$ may be thought of as a global section of
$\Hom(\mathscr E,\mathscr E\otimes\mathscr K)$. Moreover, global
sections are unchanged under pushforwards. Hence the following technical
lemma is going to be of great value:

\begin{lemma}\label{lem:linear_scheme}
  Let $S$ be a reduced scheme and $\mathscr F$ a coherent sheaf over
  $S\times C$, flat over $S$. Then the functor on $(\Sch/S)^{op}$ given by
  \begin{align*}
    (f : T\to S) \mapsto \Gamma(T,\pi_*(\identity\times f)^*\mathscr F)
  \end{align*}
  is representable.
  \begin{proof}
...
We consider the universal sheaf $\hat{\mathscr E}$ on
$Q(r,d)\times C$ arising from the universal quotient
\begin{align*}
  \hat q : \mathscr O^{\oplus n}_{Q(r,d)\times C} \to \hat{\mathscr E}
\end{align*}
in $Q(r,d)$. The lemma \ref{lem:linear_scheme} tells us
that the functor sending $T$-points $x \in \mathcal Q(r,d)(T)$
to sections $\Gamma(T,\pi_*(\identity\times x)^*\hat{\mathscr E})$
is representable. In particular, there is a map of schemes
$\mathcal F(r,d)\to\mathcal Q(r,d)$

\begin{example}
  Certain Higgs sheaves also correspond to closed points of 
  a scheme related to a Quot scheme.
  \begin{align*}
    (\Sch/Q)^\text{op} \to \Set \\
    (f : T \to Q) \mapsto 
  \end{align*}
...
\begin{example}
  ...
  and $[x]\in\projective{n}{}(\mathbb C) \cong (\mathbb C^n - \{0\})/\mathbb C^\times$.
\end{example}
...
\begin{example}
  Consider the action of $GL_{n+1}$ on $\projective{n}{}$. The
  line bundle corresponding to the twisting sheaf $\mathscr O(1)$
  provides a somewhat trivial linearisation of the action.

  More generally, suppose we have a $G$-equivariant embedding
  $X\inc\projective{n}{}$, i.e. a homomorphism $G\to GL_{n+1}$
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  Then pulling back $\mathscr O(1)$ along the embedding
  gives a linearisation. Actions on projective schemes that arise
  from the $GL_{n+1}$ action on subschemes $X\subseteq\projective{n}{}$
  in this fashion are called \emph{linear}.
\end{example}
...
\begin{lemma}
  If $L$ is a linearisation then so is $L^{\otimes k}$ for all $k\geq 1$.
  \begin{proof}
    \missingproof
  \end{proof}
\end{lemma}
...
Consider an ample linearisation $L$ of the action $\sigma : G\times X\to X$
...
$\Proj R - N(f) \to \Proj S$ where the null cone $N(f)\subseteq\Proj R$ is the closed
...
Now recall that
\begin{align*}
  X \cong \Proj\bigoplus_{k\geq 0}\Gamma(X,L^{\otimes k}).
\end{align*}
This essentially follows from \cite[Proposition 13.48]. Hence
it is natural to consider the graded algebra of sections
\begin{align*}
  R(X,L) := \bigoplus_{k\geq 0}\Gamma(X,L^{\otimes k})
\end{align*}
and the corresponding invariants
\begin{align*}
  R(X,L)^G := \bigoplus_{k\geq 0}\Gamma(X,L^{\otimes k})^G.
\end{align*}
Thus the quotient is going to arise from the inclusion
$\iota : R(X,L)^G\inc R(X,L)$. We call the points out of the null cone
semi-stable:

\begin{definition}
  A point $x\in X$ is \emph{semi-stable} if there exists a section
  $s\in\Gamma(X,L^{\otimes k})^G$ for some $k>0$ such that $s(x)\neq 0$.
  Denote by $X^{ss}(L)$ the set of all semi-stable points of $X$
  with respect to the linearisation $L$.
\end{definition}

Verifying that $N(\iota) = X - X^{ss}(L)$ is a matter of unpacking definitions.
Hence we obtain a good quotient of semi-stable points in the natural way:

\begin{definition}
  The \emph{projective GIT quotient} of $X$ with respect to $L$ is the map
  \begin{align*}
    X^{ss}(L) \longrightarrow X \sslash_L G := \Proj R(X,L)^G
  \end{align*}
  corresponding to the inclusion $R(X,L)^G\inc R(X,L)$.
\end{definition}
...
\begin{example}
  This allows us to define the moduli spaces that we
  are after:
  \missingexample
\end{example}
...
Now consider the GIT quotient $\phi : X^{ss}(L)\to X\sslash_L G$.
It follows from the definition \ref{def:good_quotient} that
$\phi(X^s(L))$ is open in $X\sslash_L G$.

\begin{theorem}
  The GIT quotient $\phi : X^{ss}(L) \to X\sslash_L G$
  restricts to a geometric quotient
  \begin{align*}
    \phi : X^s(L) \to X^s\sslash_L G := \phi(X^s(L)).
  \end{align*}
  \begin{proof}
    \missingproof
  \end{proof}
