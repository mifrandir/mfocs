...
\begin{theorem}
  ...
  \begin{proof}[Proof idea]
    ...
    into projective space is an immersion and that its image is
    closed. For an extensive overview see \cite[70-73]{hoskins2016}.
  \end{proof}
\end{theorem}
...
\begin{example}\label{ex:quot_scheme_of_lf}
  ...
  \end{align*}
  Moreover, if $q : \mathscr O^{\oplus n}_C \surj \mathscr E$ is
  a closed point of $Q(r,d)$ such that $\mathscr E$ is locally
  free and globally generated then there exists an isomorphism
  $\Gamma(X,\mathscr E)\cong\mathbb C^n$ such that $\mathscr E$ corresponds
  to $q$.
\end{example}
...
\begin{example}\label{ex:quot_scheme_of_higgs}
  ...
  Consider the universal sheaf $\mathscr U$ on $\mathcal Q(r,d)$
  arising from the universal quotient
  \begin{align*}
    \hat q : \mathscr O^{\oplus n}_{C\times\mathcal Q(r,d)} \to \mathscr U.
  \end{align*}
  Applying the lemma \ref{lem:linear_scheme} to the sheaf
  $\Hom(\mathscr U,\mathscr U\otimes K)$ on $C\otimes\mathcal Q(r,d)$
  ...
  \end{align*}
  where $\pi : C\times\mathcal Q(r,d)\to C$ is represented by a
  $\mathcal Q(r,d)$-scheme $\mathcal F(r,d)$.
  In particular, a closed point of $\mathcal F(r,d)$ corresponds to a
  closed point $f$ of $\mathcal Q(r,d)$ and an $f$-point of $\mathcal F(r,d)$.
  We may now choose $f$ such that $\mathscr E = f^*\mathscr U$ and note
  that
  \begin{align*}
  ...
  $\mathcal F(r,d)$. \todo{make sure we don't need the fact that
    $\mathcal F(r,d)$ is linear}
\end{example}
...
\begin{example}\label{ex:lf_action}
  ...
  $\mathcal Q(r,d)$, we had to choose an isomorphism
  $\mathbb C^n \cong \Gamma(C,\mathscr F)$. In particular, there
  is an action of $GL_n$ on closed points of $\mathcal Q(r,d)$
  given by $g\cdot q = q\circ g^{-1}$ for all $g\in GL_n(\mathbb C)$
  and $q : \mathscr O^{\oplus n}_C \surj \mathscr E$.

  To extend this to a schematic action $GL_n \times \mathcal Q(r,d) \to \mathcal Q(r,d)$
  we need to specify a family of quotients
  \begin{align*}
    \mathscr O^{\oplus n}_{C\times GL_n\times\mathcal Q(r,d)}\surj \mathscr E
  \end{align*}
  of rank $r$ and degree $d$. We construct such a family by
  considering the universal family $q_{\mathscr U}:\mathscr Q^{\oplus n}_{C\times\mathcal Q(r,d)}\surj \mathscr U$,
  pulling it back along the projection $GL_n\otimes\mathcal Q(r,d)\to\mathcal Q(r,d)$,
  ...
  by
  \begin{align*}
    \mathscr O^{\oplus n}_{C\times GL_n\times\mathcal Q(r,d)}
...
\begin{lemma}\label{lem:orbits_are_iso_classes}
  ...
  to closed points $q,q'$ in $\mathcal Q(r,d)$ such that
...
\begin{example}
  The $GL_n$ action on $\mathcal Q(r,d)$ lifts to $\mathcal F(r,d)$.
  In particular, we use \ref{lem:orbits_are_iso_classes} to make sure
  that
  \begin{align*}
    \mathcal F(r,d)(q) \cong \mathcal F(r,d)(g\cdot q)
  \end{align*}
  where we view $q$ as a $\mathbb C$-point of $\mathcal Q(r,d)$. Hence
...
\begin{example}
  Let us define a linearisation for the $GL_n$-action on
  $\mathcal Q(r,d)$ from \ref{ex:lf_action}. The key observation
  here is that, by construction of the action, we have equivalent
  families of quotients
  \begin{align*}
    \mathscr O^{\oplus n}_{C\times\mathcal Q(r,d)\times GL_n}
  ...
  \begin{align*}
    ...
    \pi^*\mathscr U
  \end{align*}
  over $GL_n\times\mathcal Q(r,d)$ where 
  $q_{\mathscr U} : \mathscr O^{\oplus n}_{C\times\mathcal Q(r,d)\times GL_N} \surj \mathscr U$
  is the universal quotient in $\mathcal Q(r,d)$. In particular, we have
  an isormorphism of sheaves
  \begin{align*}
    (\sigma\times\identity)^*\mathscr U \cong \pi^*\mathscr U.
  \end{align*}
  It is straightforward to verify that this extends to the invertible sheaf
  \begin{align*}
    \mathscr L^m_h := \det(\pi_*(\mathscr U \otimes \pi^*\mathscr O_X(m)))
  \end{align*}
  on $\mathcal Q(r,d)$. That is, there is an isormorphism
  \begin{align*}
    \Phi:\sigma^*\mathscr L^m_h \cong \pi^*\mathscr L^m_h
  \end{align*}
  of sheaves on $GL_n\times\mathcal Q(r,d)$ such that
  \begin{align*}
    (\mu\times\identity_C)^*\Phi = \pi^*\Phi \circ (\identity_{GL_n}\times\sigma)^*\Phi
  \end{align*}
  where $\mu$ is the multiplication on $GL_n$. Now $\Phi$ is just the
  sheaf theoretic analogue of a linearisation ,
  meaning that under the
  correspondence of locally free sheaves and vector bundles
  $\mathscr L^m_h$ and $\Phi$ define a linearisation $L^m_h$
  for all $m$. \cite[Remark 5.16]{hoskins2016}
  \todo{actually, we need $m$ large}
\end{example}
...
$\iota : R(X,L)^G\inc R(X,L)$. We call the points outside the nullcone
semi-stable:
...
In particular, $X^{ss}(L)$ is open in $X$. Hence we obtain a good quotient
...
\begin{example}
  ...
  \begin{align*}
    M^{ss}_h(r,d) := Q(r,d)\sslash_{L^m_h} G(r,d), \hspace{1cm}
    M^{ss}_H(r,d) := F(r,d)\sslash_{L^m_H} G(r,d).
  \end{align*}
  \missingexample
\end{example}
