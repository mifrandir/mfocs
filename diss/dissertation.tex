...
That is,
\begin{align*}
  U(E,h) := \{A \in\Gamma^\infty(C,\End E) : AA^\dagger = A^\dagger A = I\}.
\end{align*}
...
This once again gives rise to a Banach manifold $\mathcal A_\Phi^{ss}(E,h)_\ell$
and, for sufficiently large $\ell$, an action by $G(E,h)_{\ell+1}$.

\begin{lemma}
  Let $(\nabla,\Phi)\in\mathcal A_\Phi^{ss}(E,h)_\ell$. Then the tangent space
  \begin{align*}
    T_{\nabla,\Phi}(G(E,h)_{\ell+1}\cdot(\nabla,\Phi))
    \subseteq T_{\nabla,\Phi}\mathcal A_{\nabla,\Phi}^{ss}(E,h)_\ell.
  \end{align*}
  is a closed subspace.
  \begin{proof}
    \missingproof
...
the kernel of the elliptic operator \todo{finish}
\begin{align*}
  \hat\nabla : \Omega^1_{\mathbb R}
\end{align*}
...
\begin{align*}
  ...
  \mathcal M^{ss}_\Phi(E,h) := \mathcal A^{ss}_\Phi(E,h)_\ell/G(E,h)_{\ell + 1}.
\end{align*}
