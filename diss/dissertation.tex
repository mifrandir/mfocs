...
Unfortunately, we are not yet ready to apply this theorem as
the actions of $G(E,h)_{\ell+1}$ on $A^\nabla(E,h)_\ell$ and
$A^\Phi(E,h)_\ell$ are not free. In the next section we are
going to rectify this by restricting the spaces sufficiently.
Indeed, our argument in (\ref{ex:not_free}) already tells us what
a correct subsets to consider will be.

\subsection{Irreducible connections}
