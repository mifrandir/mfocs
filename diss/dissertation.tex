...
\begin{definition}
  ...
  which is $G$-equivariant, i.e. making the following commute:
  \begin{equation*}
...
\begin{lemma}\label{lem:rxl}
  Let $L$ be a linearisation of an action of an affine algebraic
  group on a projective scheme $X$. Then
  \begin{align*}
...
$\Proj R - N \to \Proj S$ where $N\subseteq\Proj R$ is the closed
subscheme defined by \missingcitation
\begin{align*}
  N := \{ p \in \Proj R : S_+ \subseteq f^{-1}p\}.
\end{align*}
the inclusion $R^G \inc R$ only yields a map
\begin{align*}
  \Proj(R) - N \longrightarrow \Proj(R^G)
\end{align*}
where $N\subseteq\Proj R$ is a closed subscheme sometimes referred
to as the null cone. This means that we can, in general, only hope to
obtain a good quotient on an open subscheme. In light of
\ref{lem:rxl}, we consider the graded algebra of sections
\begin{align*}
  R(X,L) := \Proj\bigoplus_{k\geq 0}\Gamma(X,L^{\otimes k}).
\end{align*}
We now call the points outside the null cone in $\Proj R(X,L)$
semi-stable:
