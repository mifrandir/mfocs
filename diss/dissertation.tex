...
\end{align*}
If it is possible to express $P$ in this way for every $x$, then we
say that $P$ is a differential operator:
...
\begin{definition}
  ...
  $\restrict{P}{U}$ may be expressed as (\ref{eq:local_diff_operator}).
\end{definition}

We want the matrix defined by the $f_I$ in (\ref{eq:local_diff_operator})
to be invertible. To make this precise in a way that respects
...
\begin{lemma}
  ...
  \begin{proof}
    Write $\Omega^k = \Omega^k_{\mathbb R}(\mathfrak u(E,h))$.
    ...
    one easily sees to restrict to $\mathfrak u(E,h)$.
...
\begin{lemma}
  ...
  \begin{proof}
    Analogous to \ref{lem:free_action}.
  \end{proof}
\end{lemma}
