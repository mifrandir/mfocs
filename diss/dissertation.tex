...
\end{align*}
Analogous to $\mathcal A_{\dol} (E)$, this is an affine space. To see what it is
...
\begin{lemma}\label{lem:real_vector_space_iso}
  ...
  \begin{proof}
    ...
    given by $A \mapsto A-\mathcal A^\dagger$.
  \end{proof}
\end{lemma}
...
\begin{example}
  ...
  \begin{align}\label{eq:nabla_hat}
    \hat\nabla := \nabla + \nabla^\dagger :
    \Omega^1_{\mathbb R}(\mathfrak u(E,h)) \to
    \Omega^2_{\mathbb R}(\mathfrak u(E,h)) \oplus
    \Omega^0_{\mathbb R}(\mathfrak u(E,h))
  \end{align}
  where we have extended the connection in $\nabla$ to a connection
  in $\End E$ using (\ref{ex:connection_on_end}) which
  one may verify restricts to $\mathfrak u(E,h)$. This is a
...
\begin{example}
  ...
  \begin{align*}
    \mathcal A_\Phi(E,h)_\ell := \mathcal A_\Phi(E,h) + \Omega^1_{\mathbb{R}}(C,\mathfrak u(E))_\ell
...
\begin{definition}\label{def:elliptic_operator}
  ...
  $\rank E = \rank F$ and, for all $x\in X$, all $s\in\Omega^0(E)$,
...
\begin{example}
  ...
  \begin{align*}
    \hat\nabla = \nabla + \nabla^\dagger :
    \Omega^0_{\mathbb R}(\mathfrak u(E,h)) \oplus
    \Omega^2_{\mathbb R}(\mathfrak u(E,h)) \to
    \Omega^1_{\mathbb R}(\mathfrak u(E,h)).
  \end{align*}
  In particular,
  \begin{align*}
    \im\hat\nabla_\ell = \im\nabla_\ell + \im\nabla_\ell^\dagger
  \end{align*}
  is closed. We now note that we have a complex \missingproof
  \begin{equation*}
    % https://q.uiver.app/#q=
    \begin{tikzcd}
      0 & {\Omega^0_{\mathbb R}(\mathfrak u(E,h))} & {\Omega^1_{\mathbb R}(\mathfrak u(E,h))} & {\Omega^2_{\mathbb R}(\mathfrak u(E,h))} & 0
    ...
    \end{tikzcd}
  \end{equation*}
  which extends to Sobolev spaces.  Noting $\im\nabla_\ell^\dagger$
  naturally is $W_{2,\ell}$-orthogonal to $\ker\nabla_{\ell}=\im\nabla_{\ell+1}$
  in $\Omega^1_{\mathbb R}(\mathfrak u(E,h))_\ell$, we find that
  $\im\nabla_\ell$ and $\im\nabla_\ell^\dagger$ must be individually
  closed. \todo{doublecheck the $\ell$s here.}
\end{example}
...
formal adjoint \todo{hermitian or Hermitian?}
\begin{align*}
...
\begin{align*}
  ...
  = i\alpha\otimes I,
\end{align*}
i.e. $\mathcal A_\nabla^{ss}(E,h)\subseteq \mathcal A_\nabla(E,h)$ is $U(E,h)$-invariant.
Thus we have a proper $G(E,h)$-action on $\mathcal A_\nabla(E,h)$.

\begin{lemma}
  The Sobolev completion $\mathcal A^{ss}_\nabla(E,h)_\ell$ is a
  Banach submanifold of $\mathcal A_\nabla(E,h)_\ell$.
  \begin{proof}
    This is a rather straightforward argument using an analogue
    of the regular value theorem for Banach manifolds. The map
    to consider is the moment map for the $G(E,h)$-action on
    $\mathcal A_\nabla(E,h)_\ell$ considered as a symplectic Banach manifold.
  \end{proof}
\end{lemma}
Just as in the finite dimensional case, the proper $G(E,h)_{\ell+1}$
action restricts to $\mathcal A^{ss}_\nabla(E,h)_\ell$.
Most importantly, we have the following:
\begin{lemma}
  Let $\nabla\in \mathcal A^{ss}_\nabla(E,h)_{\ell}$. Then the tangent space
  \begin{align*}
    T_\nabla(G(E,h)_{\ell+1}\cdot\nabla) \subseteq T_\nabla A(E,h)_\ell
  \end{align*}
  is a closed subspace.
  \begin{proof}
    This is an elliptic differential operator. \todo{justify}
    By elliptic regularity \missingcitation, the adjoint
    \begin{align*}
      \hat\nabla^\dagger := \nabla^\dagger + \nabla :
      \Omega^2 \oplus
      \Omega^0 \to
      \Omega^1
    \end{align*}
    is also elliptic and hence extends to a Fredholm operator
    \begin{align*}
      \hat\nabla^\dagger :
      \Omega^2_\ell \oplus
      \Omega^0_\ell \to
    ...
    \begin{align*}
      ...
      \Omega^0_\ell)
      =
      \nabla^\dagger\Omega^2_\ell \oplus
      \nabla\Omega^0_\ell.
    \end{align*}
    Is the image of a Fredholm operator, hence closed.
    \missingproof
  \end{proof}
\end{lemma}
...
Now consider the space $\mathcal A_\Phi^{ss}(E,h)$ of Hitchin connections.
We once again have a Banach manifold
\begin{align*}
  \mathcal A_\Phi^{ss}(E,h)_\ell := \mathcal A_\Phi^{ss}(E,h) +
\end{align*}

\subsection{Reducible Connections}

The second thing we need to ensure is that gauge transformations act freely on
our space of connections. We are going to restrict $\mathcal A^{ss}_\nabla$ and
$\mathcal A^{ss}_\Phi$ further to ensure that this is the case. Of course we will have
...
completions
\begin{align*}
  \mathcal A^s_\nabla(E,h)_\ell \subseteq \mathcal A^{ss}_\nabla(E,h)_\ell,\hspace{1cm}
  \mathcal A^s_\Phi(E,h)_\ell \subseteq \mathcal A^{ss}_\Phi(E,h)_\ell.
\end{align*}
...
\begin{lemma}
  ...
  restricts to a free action on $\mathcal A^{ss}_\nabla(E,h)_\ell$.
  \begin{proof}
...
\begin{table}[]\label{table:unitary_connection_spaces}
  \begin{tabular}{|l|l|lll|}
    ...
    $\mathcal A^{ss}_\Phi(E,h)$      & \& Hitchin                         & \multicolumn{1}{l|}{yes}              & \multicolumn{1}{l|}{no}             & yes                      \\ \hline
...
that we are after. Choose $\ell > $\todo{finish this}
\begin{align*}
  \mathcal M^s_\nabla(E,h) := \mathcal A^s_\nabla(E,h)/G(E,h)_\ell, \hspace{1cm}
  \mathcal M^s_\Phi(E,h) := \mathcal A^s_\Phi(E,h)/G(E,h)_\ell.
\end{align*}
For completeness, we are also going to define the moduli spaces of
possibly reducible harmonic connections:
\begin{align*}
  \mathcal M^{ss}_\nabla(E,h) := \mathcal A^{ss}_\nabla(E,h)/G(E,h)_{\ell + 1}, \hspace{1cm}
  \mathcal M^{ss}_\Phi(E,h) := \mathcal A^{ss}_\Phi(E,h)/G(E,h)_{\ell + 1}.
\end{align*}
