...
Although the action of any group on any set may be free, it is a particularly
important property in a geometric setting. In particular, it ensures that the
orbits are homogeneous and hence
all points in the quotient are locally indistinguishable, as one expects from
manifolds.
...
\begin{example}
  ...
  In complete analogy, there is an action of $G(E,h)$ on $A^\Phi(E,h)$ which
  is not free either.
\end{example}
...
\begin{example}
  The Lie algebra of $W_{2,{\ell}}(C,U(E,h))$ is
...
\begin{example}
  ...
  $U(1)$ acts on the Banach Lie group $U(E,h)_\ell$ by
...
\begin{example}\todo{define curvature}
  ...
  is the map
  \begin{align*}
    \mu : A^\nabla(E,h)_\ell \to \Omega^0_{\mathbb{R}}(\End E)_{\ell+1}^*
  \end{align*}
  given by
  \begin{align*}
...
\begin{example}
  ...
  \begin{align*}
    M^\omega(E) := A^\Phi (E,h)_\ell \sslash G_{\ell + 1}(E,h).
  \end{align*}
\end{example}
