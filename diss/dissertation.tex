...
\begin{example}
  ...
  \begin{align*}
    d + d^* : \Omega^1_{\mathbb R}(C) \to \Omega^2_{\mathbb R}(C) \oplus \Omega^0_{\mathbb R}(C).
  \end{align*}
...
\begin{example}
  Let $\nabla\in A_\nabla(E,h)$ be a connection.  Consider the operator
...
We will only care about top-order terms. It is going to turn out that
requiring the matrix thus defined by the $f_I$ in (\ref{eq:local_diff_operator})
to be invertible is a sufficient condition for the operator to be
very well-behaved with respect to Sobolev completions. To make this precise
in a way that respects
change of coordinates, one usually refers only to the top-order terms.
However, it is also possible to express this in a coordinate free way:

\begin{definition}
  A differential operator $P:E\to F$ is \emph{elliptic} if
  $\rank E = \rank F$ and, for all non-vanishing $s\in\Omega^0(E)$
  and all $f\in C^\infty(X)$ with $f(x) = 0$ but $df(x) \neq 0$,
  it holds that $P(f^m s)(x) \neq 0$.
\end{definition}
