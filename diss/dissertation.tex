...
\begin{definition}
  ...
  if, and only if, $\nabla\in\Omega^1_{\mathbb{R}}(C,\mathfrak u(E))$.
\end{definition}

Note that there is a different notion of compatibility with the metric.
Often one requires that, for all $s,t\in\Omega^0(X,E)$,
\begin{align*}
  dh(s,\bar t) = h(\nabla s,\bar t) + h(s,\overline{\nabla t}).
\end{align*}
It turns out that this condition is satisfied if, and only if, $\nabla$
is $h$-unitary. \cite[10-11]{kobayashi1987} For our purposes,
thinking of unitary connections as $\mathfrak u(E)$-valued $1$-forms
makes more sense. This is because are going to think of the space
$A^\nabla$ of such $h$-unitary connections as an affine space modelled on
$\Omega^1_{\mathbb{R}}(C,\mathfrak u(E))$.
...
\begin{lemma}\label{lem:real_vector_space_iso}
  ...
  \begin{proof}
    ...
    $\mathfrak u(n)_{\mathbb{C}}\cong\mathfrak{gl}_n(\mathbb{C})$.
  \end{proof}
\end{lemma}

The fact that $A^{\dol}$ and $A^\nabla$ both are affine spaces modelled
on the same underlying real vector space is not a coincidence. Indeed,
in the presence of $h$, every Dolbeault operator has a compatible
connection:
...
to help us introduce a K\"ahler structure on $A^{\dol}$ and hence
take a K\"ahler quotient. Let us do the same for the space of Higgs
bundles $A^\phi$. We are going to identify $A^{\phi}$ with an affine
space modelled on $\Omega^1_{\mathbb{R}}(C,\mathfrak u(E))^{\oplus 2}$.
This will lead to a Hyperk\"ahler structure and corresponding
quotient. Once again fix a hermitian metric $h$ on $E$.
...
connection in $E$, and let $k\geq 0$ and $1<p<\infty$ be integers.
Here we are going to think of $\nabla$ as a derivative of sections and
define a space $L^p_k(X,E)$ which may be thought of as the space of
functions $X\to E$ with $k$ derivatives, all of which in $L^p$. As
...
of $E$ and $T^*X$, respectively, and hence on all $\Omega^k(X,E)$.
...
\begin{definition}
  Define the
  \emph{$(p,k)$-Sobolev norm} on $\Omega^0(X,E)$ by
  \begin{align*}
    \Vert s\Vert_{p,k} := \left(
      \sum_{i=0}^k \int_X \Vert \nabla^i s \Vert d\text{vol}_X
    \right)^{1/p}.
  \end{align*}
\end{definition}
...
\begin{example}
  Our main application will be the following.
\end{example}

Of course Banach manifold come with all the bells and whistles that
