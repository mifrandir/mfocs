...
\section{Monoids are categories}\label{sec:monoids_are_categories}
...
\begin{definition}
  ...
  \begin{equation}\label{eq:monoid_axioms}
    % https://q.uiver.app/#q=WzAsNCxbMCwwLCJJXFxvdGltZXMgTSJdLFsxLDAsIk1cXG90aW1lcyBNIl0sWzEsMSwiTSJdLFsyLDAsIk1cXG90aW1lcyBJIl0sWzAsMSwiZVxcb3RpbWVzIE0iXSxbMSwyLCJtIl0sWzAsMiwiXFxsYW1iZGEiLDJdLFszLDEsIk1cXG90aW1lcyBlIiwyXSxbMywyLCJcXHJobyJdXQ==
    \begin{tikzcd}
...
\begin{definition}
  ...
  \begin{equation}\label{eq:monad_map_laws}
    ...
    \begin{tikzcd}
      {T\circ T} && {T'\circ T'} &&& {\text{Id}} \\
      T && {T'} && T && {T'}
      \arrow["{\phi\circ\phi}", from=1-1, to=1-3]
...
Here the natural transformation $\phi\circ\phi : T\circ T\to T'\circ T'$
has components
\begin{align*}
  T(TX)
  \xlongrightarrow{\phi_{TX}} T'(TX)
  \xlongrightarrow{T'\phi_X} T'(T'X)
\end{align*}
